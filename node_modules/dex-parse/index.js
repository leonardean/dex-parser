...
var parseEA1 = function (line, context) {
    var parts = line.split("*")
    context.machine = context.machine || {}
    context.machine.event = context.machine.event || {}

    assign(parts, 1, context.machine.event, "eventIdentification")
    assign(parts, 2, context.machine.event, "dataOfEventOccurance")
    assign(parts, 3, context.machine.event, "timeOfEventOccurance")
    assign(parts, 4, context.machine.event, "eventDurationInMinutes")
    assign(parts, 5, context.machine.event, "eventDurationInMilliseconds")
}
...
var parseEA2 = function (line, context) {
    ...
    assign(parts, 1, context.machine.event, "eventIdentification")
    assign(parts, 2, context.machine.event, "numberOfEventsSinceLastReset")
    assign(parts, 3, context.machine.event, "numberOfEventsSinceInstallation")
    assign(parts, 4, context.machine.event, "userDefinedFieldEA2")
    assign(parts, 5, context.machine.event, "eventActivity")
    assign(parts, 6, context.machine.event, "eventDurationInMinutes")
}
